...
(function() {
  if (typeof window.fetch === 'function') {
    ...
    window.fetch = function(...args) {
      const url = args[0];
      const isDataEndpoint = typeof url === 'string' && (
        url.includes('/api/') ||
      ...
      if (isDataEndpoint) {
        return promise.then(response => {
          if (!response.ok) {
            console.warn(`🚨 PRELOAD: Fetch failed for ${url} - returning empty array`);
            return {
              ok: true,
              status: 200,
          ...
          console.warn(`🚨 PRELOAD: Fetch error for ${url} - returning empty array`, error);
          return {
            ok: true,
            status: 200,
            json: () => Promise.resolve([]),
    ...
    console.log('✅ PRELOAD: Fetch patched');
  }
})();
